...
import {
  ...
  FaCheckCircle, FaTimes, FaLayerGroup, FaSearch, FaChevronLeft, FaChevronRight, FaSpinner
} from 'react-icons/fa';
...
const Gallery = () => {
    ...
    try {
      ...
      if (data.success) {
        setImages(data.data.map(img => ({
          ...img,
          id: img._id,
          url: img.imageUrl // Assuming backend returns imageUrl
        })));
      }
    } catch (error) {
  ...
  const saveUploads = async () => {
    ...
    try {
      // Assuming naive loop for now as we want individual tracking or if backend doesn't support array
      // But typically we should just send one request if possible. 
      // Given standard simple backends often take one file or array, let's try looping for safety 
      // unless we see backend code. Loops are safer for "add multiple" if endpoint is "add one".

      const uploadPromises = uploadForm.files.map((fileObj, index) => {
    ...
    >
      {loading ? (
        <div className="flex flex-col items-center justify-center min-h-[60vh]">
          ...
          <p className="mt-4 text-gray-600 font-medium tracking-widest uppercase text-[10px]">Loading Visuals...</p>
        </div>
      ) : (
        <>
                ...
                >
                  ...
                  <div className="p-10 space-y-10">
                    ...
                    <button
                      ...
                      className={`w-full py-6 bg-gray-900 text-white rounded-3xl font-black uppercase tracking-[0.2em] shadow-2xl hover:bg-black active:scale-[0.98] disabled:opacity-30 disabled:grayscale transition-all flex items-center justify-center gap-3 cursor-pointer ${uploading ? 'opacity-70 cursor-not-allowed' : ''}`}
                    >
                      {uploading ? (
                        <>
