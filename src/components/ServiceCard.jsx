...
import { FaEdit, FaTrash, FaEye, FaEyeSlash, FaExpand } from 'react-icons/fa';

const ServiceCard = ({ service, onEdit, onDelete, onToggleStatus, onView, viewMode = 'grid' }) => {
    if (viewMode === 'list') {
        return (
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            <span className={`px-2 py-1 rounded-full text-xs font-medium ${service.category === 'main'
                            ...
                            <button
                                ...
                                className={`px-2 py-1 rounded-full text-xs font-medium transition-colors ${service.isActive
        ...
        <div className="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 border border-gray-200">
            <div className="relative h-48 overflow-hidden">
                <img
                    src={service.image}
                    alt={service.title}
                    className="w-full h-full object-cover transition-transform duration-300 hover:scale-105 group-hover:scale-110"
                />
                <button
                    onClick={() => onView(service)}
                ...
                <div className="absolute top-3 right-3 flex gap-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium backdrop-blur-sm ${service.category === 'main'
                            ? 'bg-[#C6A87C]/20 text-[#C6A87C]'
                            : 'bg-[#B0956A]/20 text-[#B0956A]'
                        }`}>
                        {service.category}
                    ...
                    <button
                        onClick={() => onToggleStatus(service._id, service.isActive)}
                        className={`px-2 py-1 rounded-full text-xs font-medium backdrop-blur-sm transition-colors ${service.isActive
